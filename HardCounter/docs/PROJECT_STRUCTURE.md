...
│   └── Scene/
│       ├── Arena3DCameraController.swift
│       ├── Arena3DRingNode.swift
│       ├── ArenaPresentationGeometry.swift
│       ├── ArenaVisualPalette.swift
│       ├── ArenaViewTuning.swift
│       ├── BoxingRingNode.swift
│       ├── CombatArena3DRenderer.swift
│       ├── CombatHUDNode.swift
│       ├── CombatScene.swift
...
- `FighterProfile.swift`: 6명의 선수 식별자, 이름, 스타일, 색상과 능력치 배분을 정의한다. 모든 선수는 체력·스태미너·속도 합계 300의 동일한 예산을 사용한다.
- `FighterPortraitView.swift`: 선수별 섀시 체형, 헬멧, 센서 페이스와 장비 도장을 선택 카드의 복싱 머신 초상으로 표현한다.
...
- `CombatScene.swift`: 게임 루프와 객체 조정자다. 입력 소스가 만든 공통 명령, 이동, 화면상 히트 거리, 전투 이벤트와 경기 재시작을 연결하고 렌더링 구현은 전용 객체에 위임한다. 솔로 경기에서는 사용자가 고르지 않은 5명 중 CPU 선수를 경기 생성 시 무작위로 선택하며, 전술 판단이 사거리 경계에서 반복 취소되더라도 여유 있는 공격 데드라인이 선공을 보장한다. 첫 공격 이후의 압박·각도 변경·선제 스웨이·반응형 방어 리듬은 `CPUController`가 담당한다.
...
- `CombatTuning.swift`: 피해량, 프레임 시간, 이동 속도, 사거리, 연출 시간과 펀치·스웨이·피격 표현 진폭 등 조정 가능한 수치를 모은다. 표현 진폭은 판정 거리나 피해량을 바꾸지 않는다.
...
- `Fighter3DMaterialPalette.swift`: 선수별 대표 도장, 대비색 포인트, 중성 프레임, 보조 장갑, 발광 센서의 독립 색상 역할에서 재질을 한 번만 생성해 메시와 세부 장식이 같은 표면 설정을 공유하게 한다.
- `Fighter3DMeshFactory.swift`: 모션 리그의 관절 원점을 바꾸지 않고 장갑형 몸통·머리, 테이퍼형 팔다리, 골반 프레임과 글러브·부츠를 저폴리 메시로 생성한다. 외형 메시 실험이 전투 및 모션 계산으로 번지지 않도록 렌더러에서 분리한다.
- `FighterGroundShadowNode.swift`: 발밑 접촉 그림자와 광원 방향을 따르는 연한 투영 그림자를 합성하고 링 깊이에 따른 원근 크기를 적용한다.
- `Fighter3DMotionProfile.swift`: 여섯 선수가 공유하는 밸런스·압박·아웃복서 모션 계열의 가드 높이·기울기·리드핸드 길이·좌우 비대칭, 스탠스 깊이, 무릎 굽힘, 호흡, 보폭, 풋워크 바운스, 골반 회전, 리치, 스웨이 폭, 회복 무게와 대표 기술을 정의한다. 전투 능력치와 분리되어 모션 개성이 피해량이나 판정을 우연히 바꾸지 않는다.
- `Fighter3DMechanicalMotion.swift`: 사람형 전신 포즈 이후 복싱 머신 섀시의 질량·몸통 후행 관성·서보 관절 제한·보폭/발 들림·착지 서스펜션과 액추에이터 반응을 계산한다. 기존 보행 위상과 지지발을 입력으로만 사용하며 링 좌표와 발 IK 목표를 소유하지 않는다.
- `Fighter3DPose.swift`: 3D 관절 포즈, 캐릭터 프로필 적용, 대표 기술의 준비·타격 강조, 포즈 보간과 최종 해부학적 안전 제한을 담당한다. 렌더러의 노드 생성 코드와 독립적으로 조정할 수 있다.
...
- `FighterAppearance.swift`: 기존 프로필 필드명을 호환용으로 유지하면서 대표 도장과 별도로 프레임·보조 장갑·센서·마킹 색상 역할을 정의하고, 체형·헬멧·센서 페이스·장갑 스타일을 선수별로 구성한다.
...
- `SwayInputResolver.swift`: 버튼을 누른 순간의 스틱 화면 벡터를 정규화하고 상대 축 기준의 연속 전후·좌우 하중과 회피 가능 여부로 변환한다.
- `HapticController.swift`: 스트레이트·스매시·어퍼컷의 일반 타격, 강한 카운터, 스웨이 성공의 햅틱을 구분한다.
- `CPUController.swift`: 압박·각도 변경·거리 리셋 전술을 일정 시간 유지하며 거리별 이동과 선제 공격, 연속 공격, 방어 및 카운터 시점을 결정한다. 유효 거리에서 플레이어의 공격 시작에 반응하고, 빗나간 펀치의 회수 구간에는 얕은 각도로 거리를 닫아 공격 기회를 앞당긴다.
...
- 호스트는 왼쪽 선수, 게스트는 오른쪽 선수를 담당한다. 이동과 게스트의 로컬 액션은 즉시 예측하고, 최종 판정을 직접 수행하는 호스트의 펀치·스웨이는 2 업데이트 틱 뒤 실행한다. 호스트가 15Hz 상태 스냅샷으로 위치·체력·스태미너·승패를 교정하며, KO 후 재대결은 양쪽의 수락 투표가 모였을 때만 호스트가 실행한다.
...
- 스웨이 입력의 `screenDirection`은 화면 상·하·좌·우를 나타내는 유일한 기준이며 네트워크 전송 전후에 반전하거나 캐릭터 facing을 곱하지 않는다. `SwayDirection`은 이 벡터를 스웨이 시작 시점 상대 축의 연속 전후·좌우 성분으로 보존한다. 화면 성분은 보이는 무게중심 이동에, 상대 축 성분은 회피 판정·관절 굴곡·후속 기술 선택에 사용한다.
...
- 캐릭터 대표색은 HUD와 주 장갑에 유지하고, 프레임·보조 장갑·포인트·센서·마킹은 `FighterMachineColors`의 명시적인 역할 색을 사용한다. 외형 색 조합은 메시 배치나 모션 리그를 변경하지 않는다.
...
- `CombatScene`의 다음 구조 개선은 입력과 이동/충돌 조정을 별도 객체로 분리하는 순서로 진행한다. HUD와 3D 링·카메라·전투 프레젠테이션은 이미 전용 객체로 분리되어 있다.
