...
- `FighterNode.swift`: 폴리곤 선수 리그와 절차형 모션을 담당한다. 골반·상체를 모션 루트와 포즈 루트로 분리하고, 가드 호흡·발 디딤·체중 이동·관절별 지연을 포즈 위에 합성한다. 정면에 가까운 각도에서는 방향과 팔다리 깊이에 히스테리시스를 적용해 시각적 뒤집힘을 막는다.
