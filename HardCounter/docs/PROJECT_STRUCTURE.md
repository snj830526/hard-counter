...
- `CombatScene.swift`: 게임 루프와 객체 조정자다. 입력 소스가 만든 공통 명령, 이동, 화면상 히트 거리, 전투 이벤트와 경기 재시작을 연결하고 렌더링 구현은 전용 객체에 위임한다. 솔로 경기에서는 사용자가 고르지 않은 5명 중 CPU 선수를 경기 생성 시 무작위로 선택하며, 전술 판단이 사거리 경계에서 반복 취소되더라도 공격 데드라인이 선공을 보장한다.
