...
- 두 선수를 거리와 깊이에 맞춰 부드럽게 추적하는 공통 3D 복싱 아레나
- 사람의 체형과 팔꿈치·무릎 관절을 갖고 상대를 바라보는 로우 폴리곤 선수
- 골반·어깨·머리와 발 디딤이 연결되는 절차형 복싱 모션
- 밀어낸 거리에 따라 속도가 달라지는 아날로그 스틱
- 상황에 따라 강약이 달라지는 잽·스트레이트와 거리 기반 히트 판정
- 상대 축 기준 좌우 슬립·풀백과 카운터 판정
- 사용자가 고르지 않은 선수로 출전해 선공·접근·후퇴·사이드 스텝을 섞는 CPU
